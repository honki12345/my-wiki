...
- 요소의 화면 출력(어떻게 보여지는지)의 특성
- 기본값(block): 상자(레이아웃 요소)
- 기본값(inline): 글자요소
- 기본값(inline-block): 글자+상자요소
- 값 flex: 플렉스박스 (1차원레이아웃, 축이 하나)
- 값 grid: 그리드 (2차원 레이아웃, 행과열 축이 두개)
- 값 none: 보여짐 특성 없음, 화면에서 사라짐 -> 개발자도구에서 확인가능
...
- 한 줄의 높이, 행간과 유사
- 값 (숫자): 요소의 글꼴 크기(font-size)의 배수로 지정
- 값 (단위): px, em, rem
